<?php
function transtext()
{

	//-------------------------------------addon\content_account\recovery\display.html
	echo T_("click here to login to your account");                                   // Line 6
	echo T_("are you remember your password!?");                                      // Line 6

	//---------------------------------------addon\content_account\signup\display.html
	echo T_("already have an account?");                                              // Line 6
	echo T_("click here to sign in to your account");                                 // Line 6
	echo T_("Sign In");                                                               // Line 6

	//---------------------------------addon\content_account\verification\display.html
	echo T_("we send a verification code for you");                                   // Line 208 Seperate
	echo T_("please check your mobile and enter the code");                           // Line 7
	echo T_("don't receive message?");                                                // Line 8

	//------------------------------addon\content_account\verificationsms\display.html
	echo T_("Please send number <b>2015</b> to below number");                        // Line 6 Seperate
	echo T_("Then wait we receive your message and verificate your account");         // Line 8

	//----------------------------------------------addon\content_cp\home\display.html
	echo T_("Hello!");                                                                // Line 7
	echo T_("Welcome to Saloos control panel.");                                      // Line 7
	echo T_("Daily Visit Chart");                                                     // Line 26
	echo T_("Summarized data of site");                                               // Line 19
	echo T_("No. of");                                                                // Line 104
	echo T_("Posts");                                                                 // Line 86 Seperate
	echo T_("Pages");                                                                 // Line 87 Seperate
	echo T_("Attachments");                                                           // Line 89 Seperate
	echo T_("Tags");                                                                  // Line 112
	echo T_("Categories");                                                            // Line 99
	echo T_("Users");                                                                 // Line 91 Seperate
	echo T_("Top Visited Page");                                                      // Line 93
	echo T_("Visit");                                                                 // Line 112
	echo T_("Page Views");                                                            // Line 173 Seperate
	echo T_("Bots");                                                                  // Line 137

	//-------------------------------------------addon\content_cp\home\layout-xhr.html
	echo T_("Add New Record");                                                        // Line 79 Seperate
	echo T_("View your current location and navigate to parent of it");               // Line 68

	//-----------------------------------------------addon\content_cp\home\layout.html
	echo T_("Use this menu for navigating to all part of control panel");             // Line 8
	echo T_("Dashboard");                                                             // Line 85 Seperate
	echo T_("Books");                                                                 // Line 14
	echo T_("Visitors");                                                              // Line 18
	echo T_("Social networks");                                                       // Line 21
	echo T_("Utilities");                                                             // Line 25
	echo T_("Options");                                                               // Line 76 Seperate
	echo T_("Homepage");                                                              // Line 72 Seperate
	echo T_("Next →");                                                              // Line 38
	echo T_("← Back");                                                              // Line 38
	echo T_("Skip");                                                                  // Line 38
	echo T_("Done");                                                                  // Line 38
	echo T_("Help");                                                                  // Line 40
	echo T_("You can work with control panel in your language");                      // Line 42
	echo T_("Go to your profile or logout from conrtol panel");                       // Line 54
	echo T_("My Profile");                                                            // Line 92 Seperate
	echo T_("Change Password");                                                       // Line 198 Seperate
	echo T_("Lock Screen");                                                           // Line 94 Seperate
	echo T_("Logout");                                                                // Line 95 Seperate
	echo T_("We glad to working for you and hope you enjoy using our system");        // Line 84
	echo T_("Powered by");                                                            // Line 85
	echo T_("All right reserved");                                                    // Line 85
	echo T_("Saloos");                                                                // Line 7 Seperate
	echo T_("Last update on");                                                        // Line 86
	echo T_("Version");                                                               // Line 98 Seperate

	//-----------------------------------addon\content_cp\templates\child_display.html
	echo T_("Back to");                                                               // Line 77 Seperate

	//-------------------------------------addon\content_cp\templates\child_posts.html
	echo T_("Enter title here");                                                      // Line 19
	echo T_("Set the title of your content");                                         // Line 8
	echo T_("Url of your content appear here, click on it to view");                  // Line 10
	echo T_("Check result");                                                          // Line 12
	echo T_("Show in new tab");                                                       // Line 16
	echo T_("Write your post content here");                                          // Line 20
	echo T_("Excerpt");                                                               // Line 24
	echo T_("If excerpt is empty we get it automatically from content");              // Line 26
	echo T_("Summaries of your content that can be used as page description.");       // Line 28
	echo T_("Write your tag and press enter to add new keyword to content");          // Line 32
	echo T_("Enter Tags here");                                                       // Line 35
	echo T_("Enter Tags");                                                            // Line 36
	echo T_("Add");                                                                   // Line 130 Seperate
	echo T_("Normally, tags are ad-hoc keywords that identify important information in your post (names, subjects, etc) that may or may not recur in other posts, while categories are pre-determined sections.");// Line 15
	echo T_("View list of tags");                                                     // Line 40
	echo T_("What’s the difference between posts and pages?");                      // Line 56
	echo T_("Pages are similar to Posts in that they have a title, body text, and associated metadata, but they are different in that they are not part of the chronological blog stream, kind of like permanent posts.");// Line 56
	echo T_("Pages are not categorized or tagged, but can have a hierarchy.");        // Line 56
	echo T_("Publish");                                                               // Line 85
	echo T_("Slug");                                                                  // Line 57
	echo T_("Set slug of content for better addressing in your site and increase SEO rate");// Line 57
	echo T_("Enter slug here");                                                       // Line 67
	echo T_("Use this shortcode on your posts");                                      // Line 61
	echo T_("Language");                                                              // Line 65
	echo T_("Set language of your content to show only in this language");            // Line 66
	echo T_("All Languages");                                                         // Line 69
	echo T_("Status");                                                                // Line 74
	echo T_("All of content set as draft in default, for showing it to visitors you must set it to publish");// Line 74
	echo T_("Submit Changes");                                                        // Line 81
	echo T_("After all changes, you must press this key to Submit");                  // Line 81
	echo T_("Update");                                                                // Line 133 Seperate
	echo T_("Page Attributes");                                                       // Line 91
	echo T_("Parent Page");                                                           // Line 38
	echo T_("Select parent page to create a group of pages");                         // Line 39
	echo T_("Without parent");                                                        // Line 40
	echo T_("You can nest Pages under other Pages by making one the “Parent” of the other, creating a group of Pages.");// Line 56
	echo T_("Select a category of your content");                                     // Line 105
	echo T_("Enter cat here");                                                        // Line 109
	echo T_("Add New Category");                                                      // Line 123
	echo T_("Featured Image");                                                        // Line 129
	echo T_("This allows you to associate an image with your content.");              // Line 131
	echo T_("Enter thumb id");                                                        // Line 132
	echo T_("Set featured image");                                                    // Line 133
	echo T_("You can see type of your file and click on image to direct access to file");// Line 139
	echo T_("Click to view");                                                         // Line 148
	echo T_("Height");                                                                // Line 146
	echo T_("Width");                                                                 // Line 146

	//-------------------------------------addon\content_cp\templates\child_terms.html
	echo T_("Click to check result");                                                 // Line 11
	echo T_("What’s the difference between categories and tags?");                  // Line 15
	echo T_("If you think of your site like a book, the categories are like the Table of Contents and the tags are like the terms in the index.");// Line 15
	echo T_("You can nest category under other category by making one the “Parent” of the other.");// Line 46

	//----------------------------------addon\content_cp\templates\module_display.html
	echo T_("Actions");                                                               // Line 78 Seperate
	echo T_("Data not exist!");                                                       // Line 141 Seperate
	echo T_("delete record");                                                         // Line 80 Seperate
	echo T_("seriously, are you sure? There's no coming back.");                      // Line 81 Seperate
	echo T_("Cancel");                                                                // Line 82 Seperate
	echo T_("or");                                                                    // Line 83 Seperate
	echo T_("Delete");                                                                // Line 84 Seperate

	//----------------------------------addon\content_cp\templates\module_profile.html
	echo T_("Mobile");                                                                // Line 143 Seperate
	echo T_("Email");                                                                 // Line 13
	echo T_("Display Name");                                                          // Line 15
	echo T_("Save Changes");                                                          // Line 17
	echo T_("Edit your profile data");                                                // Line 22

	//---------------------------------addon\content_cp\templates\module_visitors.html
	echo T_("Change type of chart to");                                               // Line 15
	echo T_("bar");                                                                   // Line 16
	echo T_("area");                                                                  // Line 17
	echo T_("areaspline");                                                            // Line 18
	echo T_("column");                                                                // Line 19
	echo T_("View visitor statistics for your website");                              // Line 38
	echo T_("Pageviews");                                                             // Line 53
	echo T_("visitors");                                                              // Line 104

	//-----------------------------------------addon\content_cp\templates\sidebar.html
	echo T_("Draft");                                                                 // Line 78
	echo T_("Deleted");                                                               // Line 79
	echo T_("Schedule");                                                              // Line 80
	echo T_("Preview");                                                               // Line 83

	//--------------------------addon\content_cp\templates\static_attachments_add.html
	echo T_("Drag & Drop Files Here");                                                // Line 6
	echo T_("Choose File");                                                           // Line 6
	echo T_("Maximum size allowed");                                                  // Line 8

	//----------------------------------addon\content_cp\templates\static_utility.html
	echo T_("You can use our tools");                                                 // Line 6
	echo T_("Regenerate sitemap");                                                    // Line 10
	echo T_("Extract twig trans func in current project");                            // Line 11
	echo T_("Extract twig trans func in saloos addon");                               // Line 12
	echo T_("Automatically read database and create sql files");                      // Line 13
	echo T_("Show server info");                                                      // Line 14
	echo T_("Show PHP info");                                                         // Line 15
	echo T_("social networks");                                                       // Line 17

	//--------------------------------------addon\includes\languages\trans_static.html
	echo T_("saloos");                                                                // Line 8 Seperate
	echo T_("Powered by Saloos");                                                     // Line 9 Seperate
	echo T_("Javad Evazzadeh");                                                       // Line 10 Seperate
	echo T_("Hasan Salehi");                                                          // Line 11 Seperate
	echo T_("Ahmad Karimi");                                                          // Line 12 Seperate
	echo T_("Reza Mohiti");                                                           // Line 13 Seperate
	echo T_("Another Project with Saloos");                                           // Line 14 Seperate
	echo T_("Saloos is an artichokes for PHP programming!!");                         // Line 15 Seperate
	echo T_("Saloos is powerfull.");                                                  // Line 16 Seperate
	echo T_("Insert Successfully");                                                   // Line 17 Seperate
	echo T_("Insert Failed!");                                                        // Line 18 Seperate
	echo T_("Update Successfully");                                                   // Line 19 Seperate
	echo T_("Update Failed!");                                                        // Line 20 Seperate
	echo T_("Delete Successfully");                                                   // Line 21 Seperate
	echo T_("Delete Failed!");                                                        // Line 22 Seperate
	echo T_("Transaction error");                                                     // Line 23 Seperate
	echo T_("year");                                                                  // Line 28 Seperate
	echo T_("month");                                                                 // Line 29 Seperate
	echo T_("week");                                                                  // Line 30 Seperate
	echo T_("day");                                                                   // Line 31 Seperate
	echo T_("hour");                                                                  // Line 32 Seperate
	echo T_("minute");                                                                // Line 33 Seperate
	echo T_("second");                                                                // Line 34 Seperate
	echo T_("ago");                                                                   // Line 35 Seperate
	echo T_("s ");                                                                    // Line 36 Seperate
	echo T_("A few seconds ago");                                                     // Line 37 Seperate
	echo T_("item");                                                                  // Line 40 Seperate
	echo T_("empty");                                                                 // Line 41 Seperate
	echo T_("NaN");                                                                   // Line 42 Seperate
	echo T_("Byte");                                                                  // Line 43 Seperate
	echo T_("KB");                                                                    // Line 44 Seperate
	echo T_("MB");                                                                    // Line 45 Seperate
	echo T_("GB");                                                                    // Line 46 Seperate
	echo T_("TB");                                                                    // Line 47 Seperate
	echo T_("PB");                                                                    // Line 48 Seperate
	echo T_("id does not exist!");                                                    // Line 51 Seperate
	echo T_("all require fields must fill");                                          // Line 52 Seperate
	echo T_("some fields must be change for update!");                                // Line 53 Seperate
	echo T_("Ermile");                                                                // Line 152 Seperate
	echo T_("ermile");                                                                // Line 55 Seperate
	echo T_("made in iran");                                                          // Line 56 Seperate
	echo T_("Iran");                                                                  // Line 57 Seperate
	echo T_("iran");                                                                  // Line 58 Seperate
	echo T_("Qom");                                                                   // Line 59 Seperate
	echo T_("qom");                                                                   // Line 60 Seperate
	echo T_("submit");                                                                // Line 61 Seperate
	echo T_("save");                                                                  // Line 62 Seperate
	echo T_("add new");                                                               // Line 63 Seperate
	echo T_("signin");                                                                // Line 64 Seperate
	echo T_("sign in");                                                               // Line 65 Seperate
	echo T_("signup");                                                                // Line 66 Seperate
	echo T_("sign up");                                                               // Line 67 Seperate
	echo T_("register");                                                              // Line 68 Seperate
	echo T_("home");                                                                  // Line 69 Seperate
	echo T_("Home");                                                                  // Line 70 Seperate
	echo T_("homepage");                                                              // Line 71 Seperate
	echo T_("admin");                                                                 // Line 73 Seperate
	echo T_("Terms");                                                                 // Line 88 Seperate
	echo T_("Comments");                                                              // Line 90 Seperate
	echo T_("Profile");                                                               // Line 93 Seperate
	echo T_("Powered by Saloos.");                                                    // Line 96 Seperate
	echo T_("All right reserved.");                                                   // Line 97 Seperate
	echo T_("Comming Soon");                                                          // Line 99 Seperate
	echo T_("Fantastic");                                                             // Line 100 Seperate
	echo T_("Copy");                                                                  // Line 101 Seperate
	echo T_("Cut");                                                                   // Line 102 Seperate
	echo T_("Successfully");                                                          // Line 103 Seperate
	echo T_("Login Successfully");                                                    // Line 106 Seperate
	echo T_("Login failed!");                                                         // Line 107 Seperate
	echo T_("Mobile or password is incorrect");                                       // Line 112 Seperate
	echo T_("Please forward this message to administrator");                          // Line 113 Seperate
	echo T_("Assign keywords to your posts using tags");                              // Line 114 Seperate
	echo T_("Use categories to define sections of your site and group related posts");// Line 115 Seperate
	echo T_("Use categories to define sections of your site and group related files");// Line 116 Seperate
	echo T_("Use posts to share your news in specefic category");                     // Line 117 Seperate
	echo T_("Use pages to share your static content");                                // Line 118 Seperate
	echo T_("Upload your media");                                                     // Line 119 Seperate
	echo T_("page");                                                                  // Line 120 Seperate
	echo T_("post");                                                                  // Line 121 Seperate
	echo T_("tag");                                                                   // Line 122 Seperate
	echo T_("category");                                                              // Line 123 Seperate
	echo T_("filecategory");                                                          // Line 124 Seperate
	echo T_("File category");                                                         // Line 125 Seperate
	echo T_("File Category");                                                         // Line 126 Seperate
	echo T_("Filecategories");                                                        // Line 127 Seperate
	echo T_("attachment");                                                            // Line 128 Seperate
	echo T_("option");                                                                // Line 129 Seperate
	echo T_("Edit");                                                                  // Line 131 Seperate
	echo T_("Control Panel");                                                         // Line 132 Seperate
	echo T_("Editor");                                                                // Line 134 Seperate
	echo T_("Admin");                                                                 // Line 135 Seperate
	echo T_("Administrator");                                                         // Line 136 Seperate
	echo T_("Writer");                                                                // Line 137 Seperate
	echo T_("utility");                                                               // Line 138 Seperate
	echo T_("Utility");                                                               // Line 139 Seperate
	echo T_("Chart must be contain at least 2 column!");                              // Line 140 Seperate
	echo T_("Password");                                                              // Line 144 Seperate
	echo T_("between 5-40 character");                                                // Line 145 Seperate
	echo T_("between 5-40 character. be tricky!");                                    // Line 146 Seperate
	echo T_("Login");                                                                 // Line 147 Seperate
	echo T_("Create an account");                                                     // Line 148 Seperate
	echo T_("Change password");                                                       // Line 149 Seperate
	echo T_("SMS Delivery");                                                          // Line 150 Seperate
	echo T_("SMS Callback");                                                          // Line 151 Seperate
	echo T_("edit");                                                                  // Line 153 Seperate
	echo T_("book");                                                                  // Line 154 Seperate
	echo T_("books");                                                                 // Line 155 Seperate
	echo T_("Book Categories");                                                       // Line 156 Seperate
	echo T_("Bookcategories");                                                        // Line 157 Seperate
	echo T_("bookcategory");                                                          // Line 158 Seperate
	echo T_("Use categories to define sections of your site and group related books");// Line 159 Seperate
	echo T_("Use book to define important parts to use in posts");                    // Line 160 Seperate
	echo T_("Duplicate - File exist");                                                // Line 161 Seperate
	echo T_("Fail on tranfering file");                                               // Line 162 Seperate
	echo T_("Error");                                                                 // Line 163 Seperate
	echo T_("Invalid parameters");                                                    // Line 164 Seperate
	echo T_("No file sent");                                                          // Line 165 Seperate
	echo T_("Exceeded filesize limit");                                               // Line 166 Seperate
	echo T_("Unknown errors");                                                        // Line 167 Seperate
	echo T_("We don't support this type of file");                                    // Line 168 Seperate
	echo T_("Publish new post in social networks");                                   // Line 170 Seperate
	echo T_("Socialnetwork");                                                         // Line 171 Seperate
	echo T_("socialnetwork");                                                         // Line 172 Seperate
	echo T_("Enter your registered mobile");                                          // Line 179 Seperate
	echo T_("Enter your password");                                                   // Line 180 Seperate
	echo T_("Recovery");                                                              // Line 183 Seperate
	echo T_("your recovery code is");                                                 // Line 184 Seperate
	echo T_("recovery failed!");                                                      // Line 185 Seperate
	echo T_("Mobile number is incorrect");                                            // Line 186 Seperate
	echo T_("Verificate");                                                            // Line 189 Seperate
	echo T_("Verification");                                                          // Line 190 Seperate
	echo T_("verify successfully.");                                                  // Line 191 Seperate
	echo T_("please Input your new password");                                        // Line 192 Seperate
	echo T_("verify failed!");                                                        // Line 193 Seperate
	echo T_("this data is incorrect");                                                // Line 194 Seperate
	echo T_("Check your mobile and enter the code");                                  // Line 195 Seperate
	echo T_("Current Password");                                                      // Line 199 Seperate
	echo T_("New Password");                                                          // Line 200 Seperate
	echo T_("Change it");                                                             // Line 201 Seperate
	echo T_("change password successfully");                                          // Line 202 Seperate
	echo T_("change password failed!");                                               // Line 203 Seperate
	echo T_("we send a verification code to this number");                            // Line 209 Seperate
	echo T_("you account is verified successfully");                                  // Line 210 Seperate
	echo T_("your verification code is");                                             // Line 211 Seperate
	echo T_("your password is changed successfully");                                 // Line 212 Seperate
	echo T_("now we only support Iran!");                                             // Line 213 Seperate
	echo T_("thanks for using our service");                                          // Line 214 Seperate
	echo T_(" ");                                                                     // Line 216 Seperate

	//-------------------------------------------------addon\includes\macro\forms.html
	echo T_("Select");                                                                // Line 57

}
?>